...
window.addEventListener("DOMContentLoaded", () => {
  ...
  addDownloads(
    ["Amma Grotesk", "Bogus", "Sebra"],
    ["AmmaGrotesk", "Bogus", "Sebra"],
    ["-Regular.woff", "-Regular.otf", "-Regular.woff"]
  ); //name of fonts followed by extensions.
  ...
  function addFont(title, name, fontTypes) {
    ...
    fontTextarea.addEventListener("input", (e) => {
      ...
      for (var j = 0; j < stopList.length; j++) {
        if (
          e.target.value
            ...
            .toLowerCase() === stopList[j].toLowerCase()
        ) {
          isInStopList = true;
          break;
        }
      ...
      if (!isInStopList) {
        e.target.value =
          e.target.value.substring(0, e.target.value.length - 1) + ":L";
      }
      ...
      var parHei = e.target.parentNode.style.height.substring(
        0,
        e.target.parentNode.style.height.length - 2
      );
      // console.log(parHei)
    ...
    for (var j = 0; j < fontTypes.length; j++) {
      var opt = document.createElement("option");
      opt.value = name + "-" + fontTypes[j];
      opt.innerHTML = fontTypes[j];
